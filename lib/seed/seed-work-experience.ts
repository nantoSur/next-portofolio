...
async function seedWorkexperience() {
  try {
    ...
    console.log("✅ Data skills berhasil disisipkan");

    // ✅ Insert work experiences
    const insertedWorkExp = await sql<WorkExperience[]>`
    ...
    const workExperienceIds: WorkExperience[] = insertedWorkExp.map((item) => ({
      id: item.id,
      role: item.role,
    }));

    console.log("✅ 3 data work_experiences berhasil disisipkan");

    // ✅ Buat relasi dengan skills
    ...
    const skillsInDb = await sql<Skill[]>`SELECT id, name FROM skills`;
    ...
    console.log("✅ Relasi work_experience <-> skills berhasil disisipkan");
